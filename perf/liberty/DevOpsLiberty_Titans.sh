...
then
  ...
  echo "Release label $release has more characters than were expected.  Must be 8 or 9 characters it was $releaseLength, i.e. '20.0.0.4'|'20.0.0.12'. Please try again" >> $cronjobLog
  exit 2
fi

if [[ $releaseShort != $buildReleaseTag ]]
then
  echo "Build label tag and release label tag are not the same. Must be 'cl2004xxxx' and '20.0.0.4'.  Please try again " >> $cronjobLog
  exit 2
fi
...
SUT=titans01.rtp.raleigh.ibm.com
load_driver=titans02.rtp.raleigh.ibm.com
scriptDir=/automation/wpa_installs/wpa1/liberty/scripts
resDirName=${build_level}_$dirDate
resDir=/datastore2/results/Liberty_${release}_jdk8/linuxamd64_64/$SUT/$resDirName
...
tempRootDir=/datastore2/wpa_wlp_tmp
doDebug=false
...
echo "Transfering zipped builds to SUT" >> $cronjobLog
...
echo "Transfering scripts to SUT" >> $cronjobLog
scp -r $scriptDir/sufp root@$SUT:/ 
# Transfer pingPerf script to Load driver 
echo "Transgering script to load driver" >> $cronjobLog
scp -r $scriptDir/sufp/pingperfPingScript.sh root@${load_driver}:/sufp/ 

#Start sufp's scripts
echo "starting scripts" >> $cronjobLog
ssh root@$SUT "/sufp/sufp.sh ${build_level} ${release} ${resDir} ${resDirName} ${logFile}"

echo "Finished sufp.sh, pushing results to database" >> ${logFile}
date >> ${logFile}

echo $(date) "Finished scripts, submitting results on ${resDir} to database " >> $cronjobLog
sleep 5
