	...
	if [ -e "${APP_DEST}/${EXTRACT_NEW_NAME}" ]; then
		...
		if [ -e "${APP_ARCHIVE}" ]; then
			...
			if [ -z "${AUTH_NEEDED}" ]; then
				...
				PARTIAL_CURL_CMD="curl -OL -u ${AUTH_USERNAME}:${AUTH_PASSWORD} ${APP_URL}"
...
# Check if the Liberty build level is customized
if [[ "${LIBERTY_BUILD_LEVEL}" == "customized" ]]; then
	LIBERTY_BUILD_LEVEL=${CUSTOMIZED_LIBERTY_BUILD_LEVEL}
fi
